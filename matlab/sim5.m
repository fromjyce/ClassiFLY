...
window_size = 256;       % Increased window size for smoother spectrogram
...
subplot(3, 1, 1);
plot(t * 1e3, signal);
title('Simulated Quadcopter Time Signal (Improved)');
xlabel('Time (ms)');
ylabel('Amplitude');

% Plot power spectral density
subplot(3, 1, 2);
plot(freq, 10*log10(power_spectral_density));
title('Simulated Quadcopter Power Spectral Density (Improved)');
xlabel('Frequency (Hz)');
...
axis xy;
title('Simulated Quadcopter Spectrogram (Improved)');
xlabel('Time (ms)');
ylabel('Frequency (Hz)');
colormap('jet');
colorbar;

% Test with different window sizes for improved resolution
